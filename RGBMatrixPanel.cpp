...
volatile SemaphoreHandle_t RGBMatrixPanel::timerSemaphore;
RGBMatrixPanel* RGBMatrixPanel ::singleton;

void IRAM_ATTR RGBMatrixPanel::onTimer() {
...
void RGBMatrixPanel::drawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  */
  ...
  int16_t idx = x + y * panelwidth;
  drawBuffer()[idx] = color;
}

uint16_t RGBMatrixPanel::getPixel(int16_t x, int16_t y) {
  if (x < 0 || x >= panelwidth || y < 0 || y >= panelHeight) 0;
  int16_t idx = x + y * panelwidth;
  return drawBuffer()[idx];
}


void IRAM_ATTR RGBMatrixPanel::draw() {
  static byte cnt = 30;
  static byte y = 15;
  ...
  for (int x = 0; x < screenWidth; x++) {
    ...
    uint16_t c = matrixbuff[x + y * screenWidth];
    r1 = (c & 0x1f) > cmp;
    g1 = ((c >>  5) & 0x1f) > cmp;
    b1 = ((c >> 10) & 0x1f) > cmp;
    c = matrixbuff[x + (y + scanDevider) * screenWidth];
    r2 = (c & 0x1f) > cmp;
    g2 = ((c >>  5) & 0x1f) > cmp;
    b2 = ((c >> 10) & 0x1f) > cmp;
  ...
  out = ((y & 1) << pinA) | ((y & 2) << (pinB - 1)) |
        ((y & 4) << (pinC - 2)) | ((y & 8) << (pinD - 3)) | ((y & 16) << (pinE - 4));
  REG_WRITE(GPIO_OUT_REG, out);
